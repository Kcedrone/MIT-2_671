function [f1, params, uParams, fit_is_significant] = sine_fit(x, y, doPlots)
% If the function is run with no inputs, invent some demo data
if nargin == 0
    x = 0.1:0.05:10;
    y = 3*sin(2.2*x + 4) + 0.5*randn(size(x));
    doPlots = 1;
end
...
% The key to a good fit is good starting guesses
% Choose best starting options for sinusoidal fit
yu = max(y);
yl = min(y);
yr = (yu-yl);                               % Range of y
ym = mean(y);                               % Estimate offset
yz = y-ym;                                  % y with DC offset subtracted
zx = x(yz .* circshift(yz,[1 0]) <= 0);     % Find zero-crossings
dzx = diff(zx);                             % This is the period between zero crossings
% HF noise will cause specious zero crossings, so focus on zero crossings with long period
dzx_LF = dzx( dzx > 0.5*max(dzx) );
per = 2.*mean(dzx_LF);                       % Estimate period
...
f1 = fit(x, y, ft1, 'StartPoint', [yr/2, k, 0, ym], ...
                     'Lower', [0, 0, 0, yl], ...
                     'Upper', [yu, Inf, 7, yu]); 
