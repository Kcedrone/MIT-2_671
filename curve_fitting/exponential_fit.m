function [f1, params, uParams, fit_is_significant] = exponential_fit(x, y, doPlots)
% If the function is run with no inputs, invent some demo data
if nargin == 0
    x = -3:0.1:20;
    y = 2*exp(-x/2.5)+ 4 + 0.3*randn(size(x));
    doPlots = 1;
end
...
f1 = fit(x, y, ft1, 'StartPoint', [a_guess, b_guess, tau_guess], ...
                    'Lower', [-Inf, -Inf, 0], ...
                    'Upper', [Inf, Inf, Inf]); 
