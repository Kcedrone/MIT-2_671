function [f1, params, uParams, fit_is_significant] = arb_curve_fit(x, y, doPlots)
% If the function is run with no inputs, invent some demo data
if nargin == 0
    x = 1:0.1:20;
    y = 2*exp(0.1*x) + 3 + randn(size(x));
    doPlots = 1;
end
...
f1 = fit(x, y, ft1, 'StartPoint', [a_guess, b_guess, c_guess], ...
                     'Lower', [0, 0, 0], ...
                     'Upper', [Inf, Inf, Inf]); 
