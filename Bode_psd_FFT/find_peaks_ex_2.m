...
[freq, fft_amplitude] = plot_FFT(y, t);

%%

% Find first N_peaks peaks (N_peaks peaks at lowest freq.)
N_peaks = 6;
[pks, x_locs] = find_N_peaks(freq, fft_amplitude, N_peaks, 1);
title(sprintf('%d peaks', N_peaks));
xlabel('Frequency (Hz)');
...
function [pks, x_locs] = find_N_peaks(x, y, N_peaks, doPlots)
    ...
    [pks_raw, locs_raw, pk_widths, pk_proms] = findpeaks(y, x, ...
                                                            'MinPeakHeight', thresh);
    N_pks_found = length(pks_raw);
    ...
    %%
    pks = pks_raw(1:N_peaks);
    x_locs = locs_raw(1:N_peaks);
